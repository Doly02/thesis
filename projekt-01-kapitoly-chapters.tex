...
% uvod + konfigurace zaznamu za behu
% upravit mluvi se dvakrat o UARTu!
...
Data jsou následně v běhu záznamové úlohy vkládána do jedné ze dvou vyrovnávacích pamětí, u kterých se využívá koncept vícenásobné vyrovnávací paměti (viz. \ref{vicenasobna_vyrovnavaci_pamet}), přičemž jedna paměť vždy slouží ke sběru a předzpracování přicházejících dat, zatímco druhá slouží k jejich ukládání na paměťovou kartu prostřednictvím řadiče SDHC. Role těchto dvou pamětí jsou mezi sebou cyklicky prohazovány na základě jednoduché výměny ukazatelů. Znaky jsou tedy vkládány do sběrné vyrovnávací paměti a při vkládání je každý nový znak porovnáván s předchozím tak, aby bylo možné detekovat přítomnost znakové dvojice CR+LF z prostředí operačního systému Windows CR+LF (Carriage Return Line Feed, toto slovní spojení lze do českého jazyka přeložit jako posun o řádek a návrat vozíku). 

která zaujímá roli pro sběr a zpracování dat, zde jsou jednotlivé znaky procházeny a kontrolovány společně s předešlým znakem, zda se nejedná o speciální bílý znak z prostředí operačního systému Windows CR+LF (Carriage Return Line Feed, toto slovní spojení lze do českého jazyka přeložit jako posun o řádek a návrat vozíku). 

Při vkládání je každý nový znak porovnáván s předchozím tak, aby bylo možné detekovat přítomnost znakové dvojice CR+LF 

...
Funkcionalita digitálního záznamníku, byla prověřena pomocí skriptu využívající objektově orintovaný přístup napsaném v jazyce Python, který simulovany monitorované zařízení a to tak, že na vstup dostal množinu souboru s testovacím textem a postupně jejich obsah odesílal v podobě sériovových dat z UARTu přes COM port do digitálního záznamníku. Zařízení bylo primárně testována s rychlostmi přenosu využívající zařízení které budou v reálném světě monitorované. Testy neprokázaly žádnou vadu systému. Testovací skript se nachází v adresáři \texttt{tests/} pod názvem \texttt{serial\_test.py} a pro jejich korektní funkcionalitu je potřeba správně nastavit hodnoty definicící TESTS_FOLDER    = 'test_files'  # Relative path to the test folder

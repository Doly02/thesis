...
Modul NEO-7M obsahuje integrovaný přijímač, pasivní keramickou anténu a anténní zesilovač (nebo konektor pro připojení externí antény). Pokud jsou vyšší nároky na příjem signálu, je popřípadě možné pomocí SMA konektoru připojit aktivní anténu. Z následujícího obrázku~\ref{fig:neo-7m} je patrné, že GPS modul NEO-7M je možné připojit k mikrokontroléru pomocí vývodů TXD a RXD, které slouží pro komunikaci prostřednictvím sériového rozhraní UART. V rámci datového výstupu poskytuje také speciální vývod 1PPS (1 Pulse Per Second), který generuje přesný časový impuls synchronizovaný s GPS systémem. Tento výstup může být využit pro přesné nastavení interních hodin záznamníku a případnou kalibraci RTC obvodu DS3231M+. GPS modul se tak stává vhodným doplňkem pro aplikace, kde je vyžadována dlouhodobá přesnost času bez závislosti na ruční kalibraci či připojení k síti.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.80\textwidth]{obrazky-figures/neo-7m.png}

...
Výhodou relativních časových značek je jejich jednoduchost a vysoká přesnost v rámci sledovaného intervalu, bez nutnosti řešit komplexní problémy spojené s absolutním časem jakými jsou například přechody mezi časovými pásmy nebo sezónní změny času. Díky přímému řízení pomocí interních oscilátorů a možnosti konfigurace časovačů lze dosáhnout velmi jemné časové granularit, často v řádu jednotek mikrosekund, a v některých případech dokonce i nanosekund. To umožňuje použití relativních časových značek v aplikacích vyžadujících vysokou přesnost časování.

\newpage

\subsection{Absolutní časové značky} 
Alternativou k relativním časovým značkám, jsou značky absolutní, které obsahují plnou informaci o reálném čase události, tedy datum a čas v rámci standardizovaného časového systému, například UTC. Tento typ časových značek je nezbytný zejména tehdy, pokud je potřeba korelovat události mezi různými nezávislými systémy, kde samotný relativní čas nestačí.

Pro získání absolutního času lze v praxi využít mnoho způsobů, jakými jsou například obvod reálného času (Real Time Circuit), globální polohovací systém (Global Positioning System) poskytující velmi přesný čas synchronizovaný s atomovými hodinami, nebo pomocí například síťovou synchronizaci přes aplikační protokol NTP (Network Time Protocol). 

Příklad výsledného záznamu dat s použitím absolutních časových značek je uveden v následující ukázce~\ref{lst:absolute-timestamp}, kde v hranatých závorkách je uvedeno datum a čas události zaznamených z monitorovaného zařízení.

\begin{lstlisting}[language={}, caption={Ukázka záznamu s absolutními časovými značkami}, label={lst:absolute-timestamp}]
[2025-04-27 14:00:00] Start systemu
[2025-04-27 14:00:00] Inicializace senzoru
[2025-04-27 14:00:15] Navazani spojeni s externim zarizenim
[2025-04-27 14:00:45] Data prijata
[2025-04-27 14:02:30] Prechod zarizeni do rezimu spanku
\end{lstlisting}

Kromě výhody v již zmíněné možné korelaci událostí mezi více systémy je také využití absolutních časových značek výhodné pro zpětnou analýzu a archivaci dat v dlouhodobém horizontu.

Výhodou absolutních časových značek je možnost přesné korelace událostí v globálním časovém rámci, avšak vyžadují spolehlivý a přesný zdroj času, který je schopen kompenzovat například odchylky krystalových oscilátorů, výpadky napájení nebo síťová zpoždění. Kromě toho je potřeba řešit i specifické jevy spojené s reálným časem, například přechody mezi letním a zimním časem nebo změny časových pásem.

Přestože absolutní časové značky přinášejí řadu výhod, jejich použití může být spojeno i s určitými riziky. Může například dojít ke ztrátě přesnosti časových údajů, a to například vlivem odchylek krystalových oscilátorů, výpadků synchronizačního zdroje nebo narušením komunikačního spojení. Digitální záznamník, používající absolutní časové značky může navíc narazit na problémy i v rámci standardních situací, jako je korekce času při přechodu mezi letním a zimním časem, změna časového pásma nebo jiná úprava času, pokud zdroj času poskytující absolutní čas není správně synchronizován.
...
\label{bom_list}
Výše zmíněné obvody byly navrženy na dvouvrstvou desku plošného spoje, jejíž rozměry i rozmístění pinových lišt přesně odpovídají vývodům konektorů vývojové desky FRDM-MCXN947 od společnosti NXP Semiconductors.
...
Důležitým parametrem expanzní desky je i její cena. Celková cena komponent použitých na realizaci expanzní desky činí přibližně 880~Kč. V této kapitole je uveden přehled součástek použitých při návrhu digitálního záznamníku, včetně jejich orientačních cen. 

Nutno dodat, že doporučené pin headery od výrobce Samtec nejsou nezbytně nutné a lze je nahradit levnějšími alternativami. Všechny komponenty byly vybírány tak, aby je bylo možné objednat u jednoho distributora elektronických součástek z důvodu zjednoduššení procesu objednávky a zároveň minimalizace nákladů na dopravu.

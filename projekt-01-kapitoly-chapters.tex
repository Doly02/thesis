...
Následně jsou digitální data ukládána do paměťového média. Obvykle se pro tento účel používají sekundární paměti, tedy nevolatilní úložiště, která zaručují perzistenci dat i po vypnutí záznamníku. Typickými zástupci těchto pamětí jsou například FRAM (Ferroelectric Random Access Memory), různé typy SD karet a další technologie. Volatilní paměti, jako je například RAM, se v těchto aplikacích rovněž využívají, avšak výhradně pro krátkodobé uchování dat. V některých případech, například pokud jsou nevolatilní paměti organizovány do bloků, je výhodné nejprve dočasně ukládat data do RAM a teprve po naplnění bloku provést hromadný zápis do nevolatilní paměti.

Následně jsou digitální data ukládána do paměťového média. Obvykle se používají sekundární paměti, tedy nevolatilní úložiště, která zaručují perzistenci dat i po vypnutí záznamníku. Takovými pamětmi jsou například FRAM (Ferroelectric Random Access Memory), různé typy SD karet a další. Volatilní uložiště se používají, lze ale využít pro krátkodobé uložení nasbíraných dat, například některé nevolatilní paměti jsou organizovány do bloků, a také po blocích lze do nich zapsat, v takovém případě lze dočasně uložit data do RAM paměti, lze také využít dlouhodobé, vzdálené uložiště v podobě databáze či cloudového systému.

Uchování dat po úmyslném či neúmyslném vypnutí volatilní paměti (například operační paměť RAM) uchovávají data pouze po dobu, kdy je zařízení napájeno.
Volba úložného média je přitom klíčová nejen z hlediska kapacity a rychlosti zápisu,

Standardně se dnes používají různé typy SD karet, které poskytují relativně jednoduché připojení    přes rozhraní SPI nebo SDIO. Široce využívané jsou také paměti FRAM (Ferroelectric Random Access Memory) které kombinují výhody rychlého zápisu (pohybujícího se kolem 1–10 MB/s) a nízké spotřeby energie. Možné je také upřednostnit přístup se vzdáleným uložištěm v podobě databáze či cloudového systému.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.80\textwidth]{obrazky-figures/fat_file_system_bigger.png}

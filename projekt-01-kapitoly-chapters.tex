...
\chapter{Úvod}
Data a informace dnes představují jeden z nejcennějších zdrojů moderní společnosti. Často jsou označována jako "digitální zlato", 
protože jejich správné využití může přinést obrovskou hodnotu v různých oblastech – od vědy a průmyslu až po zdravotnictví 
a každodenní život. Aby však mohla být efektivně využita, je nutné je správně zaznamenávat, ukládat a analyzovat.

Vzhledem k široké škále oblastí, kde data vznikají, se používají různé metody jejich sběru a uchovávání. Data mohou pocházet 
z různých zdrojů – například senzorů měřících fyzikální veličiny, síťových zařízení sledujících internetový provoz, medicínských 
přístrojů monitorujících životní funkce pacientů, ekonomických systémů analyzujících finanční transakce či průmyslových strojů 
zaznamenávajících provozní parametry. Každý z těchto zdrojů má specifické požadavky na způsob sběru, formátování a uchovávání dat. 

Tato bakalářská práce je věnována návrhu a implementaci takového zařízení, které bude schopno záznamu dat, požadavek na zařízení 
vznikl od firmy NXP Semiconductors, pro tým věnující se bezdrátovému nabíjení, působící (poznámka: asi lépe to rozdělit na 
kontinenty) v České republice, jak v Brněnské, tak i Rožnovské pobočce, tak i v zahraničí v Asii a také Severní Americe. 
Tento tým se věnuje vývoji referenčních designů, které jsou následně nabízeny jako předloha zákazníkům. Zákazník tedy dostává 
téměř hotový produkt, který je již možné certifikovat v Qi certifikačních laboratořích. Zákazníci si nicméně poskytnuté návrhy 
upravují podle svých preferencí, často zaměřeny na požadavky konkrétní poptávky koncového zákazníka (RFQ, request for quotation), 
takovéto změny mohou být například realizovány z důvodu snížení ceny nebo zlepšení výkonosti - konkrétně EMC charakteristik. 
Změny u takovýchto produktů však přinášejí často nové výzvy, zákazníci dostávají plnou podporu až do okamžiku, než projdou 
úspěšnou certifikací. Zákazníci si často vybírají certifikační laboratoře různě po světě a bohužel u každé z nich nemůže být 
zaměstnanec firmy NXP Semiconductors, který by dohlédl na to, že dojde k plnému úspěchu. Častokrát se tak tým na bezdrátové 
napájení musí spoléhat na záznamy poskytnuté operátorem certifikační laboratoře. Nicméně tyto záznamy pocházejí pouze z 
přijímače -- certifikačního zařízení, nejčastěji od výrobců Nok9 či Granite River Labs - zkráceně GRL. Získané záznamy, 
jsou tedy pouze ze strany přijímače, a není v nich uveden explicitní popis chování vysílače, tedy bezdrátové nabíječky, a 
pokud tedy bezdrátová nabíječka, nějakým testem neprojde, což se občas stane, je těžké přijít na to, co konkrétně nastal za 
problém.

Jedním z požadavků na tento digitální záznamník bylo krom prevence ztráty dat při výpadku napájení i jednoduchost na použití, 
tedy aby i nezkušený operátor, který bude v Qi laboratoři dané zařízení testovat, mohl nabíječku na začátku testování zařízení 
pouze připojit k záznamníku a na konci testování zase pouze odpojit a předat ho zákazníkovi nebo přímo někomu z NXP Semiconductors. 
Jedním z dalších požadavků je možnost vyčtení získaných dat bez nutnosti vyjmutí fyzického média, jelikož pokud se na certifikace 
nevyvíjí spravným směrem, je důležité záznamy odeslat co nejdřívě, proto není možné aby zákazník poslat uložiště obsahující 
získané data.

...
Bylo tedy třeba zaintegrovat USB Mass Storage do firmwaru digitálního záznamníku, který je v rámci projektu umístěn v adresáři \texttt{msc/}, dále je součástí v rámci složky \texttt{source/}, soubor \texttt{mass\_storage.c} obsahuje aplikaci USB Mass Storage, kterou je možné rozšířit. Z pohledu Mass Storage je také důležitá obslužná rutina přerušení \texttt{USB1\_Hs\_IRQHandler()}, která zprostředkovává komunikaci a přenos dat mezi hostitelským zařízením.


Samotná implementace USB Mass Storage byla převzata z softwarového balíku MCUXpresso SDK (Software Development Kit), který poskytuje společnost NXP Semiconductors. Implementace USB Mass Storage z poskytnutého balíku je univerzální a navržená tak, aby podporovala širokou škálu mikrokontrolérů, z tohoto důvodu jsem modul optimalizoval. Například není potřeba rozsáhlá množina kódu podporující různé typy hostitelských rozhraní (Host Controller Interface - HCI) jakými jsou například EHCI, KHCI, či speciální řadiče určené pro čipy rodiny LPC.

Portace USB Mass Storage do firmware digitálního záznamníku tedy spočívala 

jejíž hlavní logika je spuštěna v případě, kdy je k digitálnímu záznamníku připojeno hostitelské zařízení pomocí USB1\_HS. USB Mass Storage je vcelku používané, i samotní výrobci čipů pro usnadnění času vývojářů poskytují tuto aplikaci, a právě to poskytuje NXP Semiconductors v rámci svého SDK (Software Development Kit). Tato implementace dokáže zpřístupnit SD kartu.

Mým úkolem bylo tedy naportovat USB Mass Storage na vyvíjenou aplikaci digitálního záznamníku, zároveň poskytnutá implementace je obecná a vyvíjená se záměrem kompatibility s velkou řadou mikrokontrolérů, proto jsem její obsah pročistil. Například poskytnutý Mass Storage poskytuje vícero rozhraní hostitelských řadičů (Host Controller Interface - HCI) jakým je například EHCI, KHCI a speciální rozhraní pro rodinu LPC. Možnosti KHCI a rozhraní pro rodinu LPC byly odstraněny, jelikož jsou nekompatibilní, a v případě portace firmwaru na jiný mikrokontrolér v rámci výrobce NXP Semiconductors je lze snadno zpět přidat.  

Kód realizující funkcionalitu USB Mass Storage je také organizován do externího modulu, proto má tento kód vlastní externí adresář \texttt{msc/}.
...
Obecně proč je potřeba testovat, jaké jsou možnosti testování a validace vestavěných systémů

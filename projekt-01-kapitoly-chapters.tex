...
Aktivní buffer je tímto způsobem postupně plněn záznamovými daty a doplňován o aktuální časové značky získané z interního obvodu reálného času. Jakmile dojde k jeho naplnění, jsou role obou vyrovnávacích pamětí vzájemně zaměněny prostou výměnou ukazatelů. Paměť, která dosud sloužila jako aktivní, je nyní určena ke zpracování a zápisu dat na SD kartu, a naopak – paměť předtím vyhrazená pro zápis se stává novým aktivním bufferem, připraveným přijímat další příchozí znaky. Samotný zápis dat do paměťového média je realizován prostřednictvím transportního algoritmu s přímým přístupem do paměti (ADMA – Advanced Direct Memory Access). \cite{nxp_MCX_Nx4x_Reference_Manual}

% je pojisteno ze kontrola CRLF probehne validne i v pripade ze je obsazeno ve dvou bufferech
Takovýto algoritmus sběru dat a jejich zpracování je ohrožen pár skutečnostmi. Může například nastat situace, kdyby speciální znak CR se nachází v jedné vyrovnávací paměti, následně dojde k prohození vyrovnávacích pamětí a tedy speciální znak LF se dostane na první pozici v nové aktivní vyrovnávací paměti. Z tohoto důvodu jsou tedy ukládány jakékoliv po sobě jdoucí znaky a tedy k této situaci nedojde. Dále je ošetřena situace u druhé problematické situaci, která je taktéž spojena se speciální dvojcí znaků CR+LF, ta nastává v případě, že tato dvojce znaků se vyskytne na úplném konci aktivní vyrovnávací paměti nebo před jejím koncem, ale časová značka se již nevleze celá do této paměti, i tato situace je tedy ošetřena a tedy zbytek časové značky, v takové situaci je část časové značky (která se vleze) uložena do původního aktivní vyrovnávací paměti, následně dojde ke prohození a zbylá část je uložena do nové (prázdné) paměti,

U tohoto algoritmu sběru dat a z jejich zpracování hrozí  dojít v průběhu k pár problémům, prvním z nich  

...
Na rozdíl od jednoduchého sekvenčního provádění kódu v nekonečné smyčce umožňuje RTOS rozdělit aplikaci do více úloh (tasks), které mohou běžet paralelně, a řídit jejich vykonávání prostřednictvím plánovače (scheduler). Ten rozhoduje, která úloha bude v daný moment vykonávána na základě priorit, časování a dalších parametrů.
...
RTOS podporuje multitasking, tedy schopnost spouštět více logických jednotek současně. Zmiňovanou nejmenší logickou jednotkou je obvykle vlákno (thread) nebo úloha (task), které soutěží o čas procesoru a jsou vykonávána schedulerem. Různé RTOS implementace mohou tyto jednotky spravovat odlišně – například FreeRTOS používá úlohy (tasks), zatímco Zephyr RTOS pracuje s vlákny (threads). Na rozdíl od obecných operačních systémů, jako je Linux, které obvykle využívají procesy oddělené pomocí virtuální paměti, RTOS často pracuje bez této vrstvy abstrakce. \cite{freertos_what_is_rtos}

Plánovač je součástí jádra (kernelu), které představuje centrální komponentu RTOS a má na starosti správu běhu jednotlivých úloh. Kromě samotného plánování a přepínání mezi úlohami (tzv. přepínání kontextu) zajišťuje jádro také podporu pro synchronizaci a komunikaci mezi úlohami (např. semafory, fronty, mutexy), správu systémového času a obsluhu přerušení. Díky těmto funkcím umožňuje RTOS deterministické a přesně načasované vykonávání úloh, což je klíčové pro vestavěné systémy s reálnými časovými požadavky.

\begin{figure}[h]
    \centering
    \includegraphics[width=1.00\textwidth]{obrazky-figures/rtos_scheduling.pdf}
    
    \caption{Graf znázorňující rozdělení výpočetního času, při řízení programu pomocí operačního systému reálného času s plánovačem úloh (scheduler)}
    \label{fig:rtos-scheduling}
\end{figure}

Operačním systémům lze vyčítat režii a nedůvěřovat jejich funkčnosti, ale nepochybně snižují náročnost programování tím, že poskytují funkce pro správu paměti a také správu procesů a umožňují programování pomocí vysokoúrovňového jazyka, jako je C
...
Bezpečnost a zabezpečení: RTOS se často používají v kritických systémech, jejichž selhání může mít katastrofální následky, například v robotice nebo v řídicích systémech letadel. Aby ochránily své okolí, musí mít vyšší bezpečnostní standardy a spolehlivější bezpečnostní funkce.

\subsubsection{FreeRTOS}
Konkrétní výhody a nevýhody FreeRTOS

\subsubsection{ZephyrRTOS}
Konkrétní výhody a nevýhody ZephyrRTOS

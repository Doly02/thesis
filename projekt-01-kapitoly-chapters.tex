...
Nejmenší logická jednotka provádění pro scheduler, je obvykle vlákno (Thread) nebo úloha (Task) která soutěží o čas procesoru.

RTOS podporuje multitasking, tedy schopnost spouštět více výpočetních jednotek současně. Zmiňovanou nejmenší logickou jednotkou je obvykle vlákno (thread) nebo úloha (task), které soutěží o čas procesoru a jsou vykonávaná plánovačem (schedulerem). Různé RTOS implementace mohou tyto jednotky spravovat odlišně – například FreeRTOS používá úlohy (tasks), zatímco Zephyr RTOS pracuje s vlákny (threads). Na rozdíl od obecných operačních systémů, jako je Linux, které obvykle využívají procesy oddělené pomocí virtuální paměti, RTOS často pracuje bez této vrstvy abstrakce,

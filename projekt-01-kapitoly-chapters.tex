...
\subsection{Nízko--energetické režimy}
...
V prvním případě jde o~zařízení zaměřená na sledování jednotlivých událostí, jako jsou překročení prahové hodnoty, uplynutí pevně definovaného intervalu nebo výskyt externího podnětu, například přerušení od senzoru. Časová značka se v~těchto případech přidává pouze v~okamžiku výskytu takové události.~\cite{datalogger_implementation_temperature, researchgate_general_dataloggger_multiple_sdcards, ieee_multi_connectivity_datalogger_sd_card}
...
\label{bom_list}
Výše zmíněné obvody byly navrženy na dvouvrstvou desku plošného spoje v~návrhovém softwaru KiCAD~8, jejíž rozměry i~rozmístění pinových lišt přesně odpovídají vývodům konektorů vývojové desky FRDM-MCXN947 od společnosti NXP Semiconductors.~\cite{KiCAD}

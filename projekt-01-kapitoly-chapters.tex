...
Na podporu dobré čitelnosti kódu a snazší orientace ve zdrojových souborech byla kromě důkladné dokumentace kladena pozornost také na jednotný systém pojmenovávání proměnných. Pojmenování proměnných reflektují datový typ, rozsah a případnou strukturu. Globální proměnné jsou označeny prefixem \texttt{g\_}, čímž se jednoznačně odlišují od lokálních. Datový typ proměnné je dále specifikován typovým prefixem — např. u32 pro uint32\_t, \texttt{u16} pro uint16\_t, \texttt{b} pro bool, \texttt{p} pro ukazatel a \texttt{a} pro pole. Proměnné typu struktura (pokud nejsou globální) jsou pojmenovávány bez prefixu. Tyto prefixy jsou vzájemně kombinovány, aby správně reprezentovaly danou proměnnou.
...
// Lokální proměnná typu struktura – bez prefixu
/**
 * @brief 	Lokální proměnná typu struktura – bez prefixu
 * @details  struktura reprezentující stav záznamu
 */

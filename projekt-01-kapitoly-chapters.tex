...
Raspberry Pi je nízkonákladový minipočítač, de facto dnes už rodina počítačů, postavených na procesorech společnosti ARM, které podporují běh různých operačních systémů. Raspberry Pi oproti klasickým mikrokontrolérům poskytuje vyšší výpočetní výkon, taktéž i větší množství operační paměti a podporuje také multitasking, umožňující běh více aplikací či procesů současně.
...
Jako nejvhodnější platforma pro implementaci digitálního záznamníku ve formě dedikovaného zařízení jsem vybral vývojovou desku FRDM--MCXN947 od společnosti NXP Semiconductors, která poskytuje široké spektrum integrovaných periferií vhodných pro realizaci digitálního záznamu a zároveň nabízí nejvyšší míru volnosti při výběru komponent, kterými může být tato vývojová deska rozšířena.

Alternativa Arduino Due, podle mého hlediska za svou cenu neposkytuje dostatečný poměr cena/výkon. Maximální taktovací frekvence jádra je pouze 84 MHz, navíc obsahuje mikrokontrolér postaven na starší architektuře Cortex-M3. Kapacita flash paměti je oproti svým levnějším konkurentům pouze 512 KB a současně postrádá slot pro microSD kartu, která by jinak mohla sloužit jako vhodné médium pro ukládání zaznamenaných dat. Také výběr, z jakého je možné vybrat řešení pro detekci ztráty napájení, je omezen.

Raspberry Pi Zero 2W naopak nabízí z vybraných vývojových desek velice dobrý poměr cena/výkon, obsahuje vysoce výkonný čip BCM2711 s taktem až 1 GHz a nabízí prostor, kde mohou být uloženy získané záznamy, zmíněný slot pro mikro SD kartu. Nicméně ve srovnání s vývojovou deskou FRDM–MCXN947 nabízí jen omezené množství dostupných periferií, což snižuje potenciál pro rozšiřování systému v podobě externích komponent.
...
\begin{table}[h]
    ...
    \begin{tabular}{|l|c|c|c|}
        ...
        \hline
        \textbf{Taktovací frekvence}    & 84 MHz                & 150 MHz               & 1 GHz               \\ 
        \hline
...
V dnešní době existuje několik variant systému FAT, mezi nejznámější patří FAT12, FAT16, FAT32 a exFAT. Pro úložná média typu SDHC karet se obvykle volí FAT32, neboť nižší varianty mají velmi omezenou maximální kapacitu úložného média, například pro FAT16 je kapacita omezena pouze na 4~GB, což je pro dnešní aplikace často nedostatečné. Z tohoto důvodu i dnešní implementace FATFS podporují 32bitovou či 64bitovou variantu adresování jednotlivých bloků dat (LBA - Logical Block Addressing), které podporují mnohem větší kapacitu uložišť. \cite{elm_fat_filesystem_app_note}
...
V předchozích kapitolách byly představeny vybrané komponenty, které lze využít k sestavení digitálního záznamníku v podobě dedikovaného zařízení. Z představených vývojových desek byla jako nejvhodnější byla zvolena deska FRDM-MCXN947 od společnosti NXP Semiconductors (viz. \ref{nxp_frdm_mcxn947}), která poskytuje dostatečný výkon, paměťový prostor a periferie, které lze využít k realizaci záznamníku.

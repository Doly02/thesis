...
Analogový komparátor je přítomen na MCU MCXN947 v podobě periferie \texttt{LPCMP} (Low-Power Comparator), který bude porovnávat napětí na vstupu (\SI{5}{\volt} větev) oproti referenčnímu napětí. Jak již bylo zmíněno v kapitole~\ref{zalohovane_napajeni_hw}, důležitá je z pohledu detekce hodnota napětí \SI{4.6}{\volt}, která je z důvodu Schottky diody standardní hodnotou napětí, jelikož je však před vstup do komparátoru umístěná ještě napěťová dělička, pak je standardní vstupní napětí do komparátoru rovno hodnotě: 
...
Perifie \texttt{LPCMP} obsahuje interní DAC, díky kterému je možné porovnávat vstupní analogový signál s 256 respektive 255 úrovněmi referenčního napětí. A tedy vzdálenost (krok) mezi jednotlivými úrovněmi činí \SI{0.01294}{\volt}, vzhledem k tomu, že jako referenční napětí je považována hodnota \SI{3.3}{\volt}.  
\begin{equation}
    Krok_{DAC} = \frac{\SI{3.3}{\volt}}{255} = \SI{0.01294}{\volt} 
    \label{eq:dac-step}
\end{equation}

Komparátor musí reagovat co nejdřív, aby MCU dostalo co nejvíce času na uložení dat do SD karty, proto jsem jako hodnotu napětí, při které bude komparátor reagovat, vybral \SI{4.5}{\volt}, respektive \SI{2.91176}{\volt}. Z tohoto důvodu je hodnota v DAC, při které bude komparátor reagovat, nastavena na hexadecimální hodnotu 0xE1, jelikož:
...
Reakce komparátoru je rovněž ovlivněna hysterézí nastavené na hodnotu \SI{20}{\milli\volt} aby byly omezeny falešné detekce. Jakmile je přerušení od komparátoru vyvoláno, obslužná rutina příslušného přerušení uloží všechna data, která jsou dočasně uložená ve volatilní paměti RAM, do SD karty pomocí funkce  \texttt{CONSOLELOG\_Flush()} a uzavře soubor, a systém je připraven k vypnutí.

V systému je ještě přítomen časovač, který aktivuje detekci ztráty napájení, jelikož se v ojedinělých případech objevily falešné detekce, pokud byl digitální záznamník napájen z měkkého zdroje a obvod pro zálohované napájení ještě nebyl plně nabit, k uvede za dobu 5 tau komparátor do původního stavu. Stav, kdy je komparátor připraven k detekci, je signalizován pomocí LED (viz. kapitola \ref{signalizace_stavu_systemu}).

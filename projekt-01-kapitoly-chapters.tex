...
Po inicializaci, ve které je získáno nastavení záznamu, jsou tyto parametry nastaveny a inicializují se periferie potřebné k záznamu. Poté je vytvořen hlavní relační adresář (Session Directory) pomocí funkce \texttt{CONSOLELOG\_CreateDirectory()}, do kterého budou ukládány záznamy z dané relace, která trvá, až do vypnutí digitálního záznamníku. Po této přípravné fázi následuje hlavní smyčka úlohy, která zprostředkovává záznam dat, ukládání dat do SDHC karty a případně signalizuje stav záznamu uživateli.

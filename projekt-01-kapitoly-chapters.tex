...
Součástí konfiguračního souboru \textit{defs.h} je rovněž možnost zvolit si, jakou mírou informačního výstupu bude firmware komunikovat s uživatelem či vývojářem. Záznamník podporuje tři úrovně výstupu zpráv. První, nejnižší úrovní, jsou chybová hlášení (začínající řetězcem \texttt{ERR:}), která informují o kritických stavech. Tato vrstva je automaticky aktivní a nelze ji vypnout.

Současně tento konfigurační soubor umožňuje konfigurovat velikost zásobníků statických úloh a případně upravit hodnoty záznamu

...
U vývoje firmwaru byl kladen rovněž důraz na dokumentovanost zdrojového kódu. Všechny funkce, globální proměnné a makra jsou opatřeny komentáři ve formátu \textit{Doxygen}, ze kterých lze snadno vygenerovat dokumentaci. Krom toho je samotný kód doplněn o kontextové komentáře, které objasňují klíčové části implementace.
...
Z důvodu prevence ztráty dat, ale také z důvodu ochrany souborového systému, z důvodu náhlého výpadku napájení, je implementována detekce ztráty napájejícího napětí. Ta je realizována za pomoci komparátoru a časovače. Tato volba mi příjde jako vhodná z toho není třeba periodicky kontrolovat napětí v systému (tzv. Polling), ale pouze pomocí přerušení, a lze tuto detekci upravovat, pouze pomocí firmwaru.

Konkrétně se pro detekci využívá periferie \texttt{LPCMP} (Low-Power Comparator), který porovnává napětí vstupu (\SI{5}{\volt} větev) oproti referenčnímu napětí, jak již bylo zmíněno v kapitole \ref{zalohovane_napajeni_hw}, důležitá je z pohledu dektekce hodnota napětí \SI{4.6}{\volt}, která je z důvodu Schottky diody, standardní hodnotou napětí, pokud je vše v pořádku, obvodu je ještě umístěna dělička napětí, díky které je možné aby MCU mohlo zpracovat signál. Perifie \texttt{LPCMP} obsahuje interní DAC, díky kterého je možné porovnávat vstupní analogový signál s 256 úrovněmi referenčního napětí. Komparátor je nastaven konkrétně na hodnotu 
...
Nedílnou součástí vývoje vestavěnéných systémů by mělo být i testování sloužící k ověření funkcionality, stability, spolehlivosti i bezpečnosti.

% ----------------------------------------------------

\section{Funkcionální testování systému}
Funkcionalita digitálního záznamníku byla prověřena pomocí skriptu využívajícího objektově orientovaný přístup napsaném v jazyce Python. Tento skript simuluje monitorované zařízení tak, že načítá testovací textové soubory a jejich obsah postupně odesílá ve formě sériových dat přes COM port do digitálního záznamníku. Testovací skript se nachází v adresáři \texttt{tests/} pod názvem \texttt{serial\_test.py} a pro jejich korektní funkcionalitu je potřeba správně nastavit hodnoty definicící, určující COM port, sadu testovacích souborů a přenosovou rychlost.

Testování bylo prováděno s různými přenosovými rychlostmi, přičemž primárně bylo zaměřeno na přenosové rychlosti odpovídající skutečným scénářům nasazení. Během funkcionálního testování nebylo pozorováva žadná ztráta dat.

\section{Kontrola bezpečnosti kódu}
Kromě funkcionálního testování byla provedena statická analýza zdrojového kódu pomocí pravidel specifikace MISRA C:2012. K tomu účelu byla využita nástroje PC-lint Plus ve verzi 2.2 od společnosti Vector s 14 denní licentí zdarma. Konfigurační soubory jsou společně s výstupy PC-lint Plus uloženy složce \texttt{test/static-analysis/}. Pozornost byla věnována primárně pravidlům spadající do kategorie Required a Mandatory.\footnote{Pravidla ze skupiny Advisory nebyly brány v potaz, tyto chyby nijak nenarušují fukcionalitu firmwaru.} Většina porušení pravidel byla opravena a nebo obhájena pomocí komentářu příčinu a důvod porušení pravidla (viz. ukázka~\ref{lst:misra}). Obhájená porušení se týkají zejména kvůli architektuře MCUXpresso SDK, FreeRTOSu či z důvodu, že PC-lint Plus nedokázal najít souvislosti mezi zdrojovými a hlavičkovými soubory.

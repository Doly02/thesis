...
Souborový systém LittleFS využívá pokročilý mechanismus atomického zpracování známého z databázových systémů, kdy je každá operace buď kompletně dokončena, nebo v případě selhání či výpadku napájení zcela zrušena.

jednotlivé zápisy  a také žurnálování (journaling), které je naopak 
Mezi další důležité vlastnosti LittleFS patří vysoká odolnost vůči výpadkům napájení, jelikož je navržen s důrazem na integritu a konzistenci dat. Díky integrovaným mechanismům, jako je žurnálování (journaling) a atomicita operací, umožňuje LittleFS automatickou obnovu souborového systému do konzistentního stavu i po náhlých výpadcích napájení. Dále LittleFS nabízí velmi nízkou režii na paměť a nenáročnost na systémové prostředky, což umožňuje jeho použití i na velmi omezených mikrokontrolérech s minimální kapacitou operační paměti.
